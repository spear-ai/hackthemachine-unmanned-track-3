...
## Airbases
airbases = pd.read_csv(r'C:\Users\tjack\Documents\HackTheMachine\synthetic_data\Points_Of_Origin_Destination\Airbases.csv')

## p8 flight
# p8flight = pd.read_csv(r'C:\Users\tjack\Documents\HackTheMachine\synthetic_data\Routes\p8_new.csv')
p8flight = pd.read_csv(r'C:\Users\tjack\Documents\HackTheMachine\synthetic_data\Routes\p8_72.csv')
...
s = len(df_fogarray)
df_fogarray = pd.concat([df_fogarray, df_fogarray], ignore_index=True)
df_fogarray['time'].iloc[s:] = df_fogarray['time'].iloc[s:] + 47

## New Ceiling
df_ceilingarray = pd.read_csv(r'C:\Users\tjack\Documents\HackTheMachine\synthetic_data\Weather\Visibility\ceilingtime3.csv')
# s1 = len(df_ceilingarray)
# df_ceilingarray = pd.concat([df_ceilingarray, df_ceilingarray], ignore_index=True)
# df_ceilingarray['time'].iloc[s1:] = df_ceilingarray['time'].iloc[s1:] + 47

## Need to set second ceiling variable for ideal altitude
## Altitude to Radar Range
# df_ceilingarray['Radar Range'] = (2 * ((df_ceilingarray['ceiling'] - 300)* 0.0003048) * ((4/3) * 6.4*10^3))^0.5
...
##Read in time
timedf = pd.read_csv(r'C:\Users\tjack\Documents\HackTheMachine\synthetic_data\Routes\timedf.csv')

## From Amy RL Tracks
# rltracks = pd.read_csv(r'C:\Users\tjack\Documents\HackTheMachine\synthetic_data\Routes\rltracks1.csv')
rltracks = pd.read_csv(r'C:\Users\tjack\Documents\HackTheMachine\synthetic_data\Routes\rltracks_cent.csv')
# rltracks = pd.merge(rltracks, timedf,  how='left', left_on=['time'], right_on = ['time'])
...
rltracks3999['Latitude'] = rltracks3999['Latitude'] 
rltracks3999['Longitude'] = rltracks3999['Longitude'] 
...
times = [i for i in range(0,86)]
quotients = []
for times in times:
    quotients.append(times / 2)
...
s2 = len(df_daylight)
df_daylight = pd.concat([df_daylight, df_daylight], ignore_index=True)
df_daylight['time'].iloc[s2:] = df_daylight['time'].iloc[s2:] + 47
...
ceilingrangechlor = {
                            ...
                            'z': df_ceilingarray['Radar Range'][df_ceilingarray['time'] == 0],
                            'locations':df_ceilingarray['ids'][df_ceilingarray['time'] == 0],
                            'colorscale': [[0, 'rgba(255,255,255, 0.0)'], [0.05, 'rgba(227,238,245, 0.4)'], [0.5, 'rgba(157,186,217, 0.5)'], [1.0, 'rgba(113,25,117, 0.6)']],
                            # 'colorscale':'purp',
                            # 'colorbar':dict(thickness:20, ticklen:3),
                ...
                #           lon=rltracks800.loc[:k+1, 'Longitude'].tolist()),
                    ...
                    dict(
                       type= "choroplethmapbox",
                        ...
                        z = df_ceilingarray['ceiling'][df_ceilingarray['time'] == (k + 4 // 2) // 4],
                        locations = df_ceilingarray['ids'][df_ceilingarray['time'] == (k + 4 // 2) // 4],
                        geojson = grid24df,
                    ...
                    dict(
                       type= "choroplethmapbox",
                        ...
                        z = df_ceilingarray['Radar Range'][df_ceilingarray['time'] == (k + 4 // 2) // 4],
                        locations = df_ceilingarray['ids'][df_ceilingarray['time'] == (k + 4 // 2) // 4],
                        geojson = grid24df,
